...
function App() {
	...
	const { minutes, hours, seconds, isRunning, start, reset, restart } = useStopwatch({ expiryTimestamp: time });
	...
	function handleSuccess(stream) {
		...
		let setIntervalDrawScreenShareId = window.setInterval(
			() =>
				drawOnCanvas({
					recording,
					screenshare: true,
	...
	function handleSuccessVideoRecording(stream) {
		...
		if (!screenshare) {
			let setIntervalDrawVideoId = window.setInterval(
				() =>
					drawOnCanvas({
						recording: true,
						screenshare,
			...
			if (window.expanded == 'screen') {
				...
				canvas
					...
					.drawImage(
						...
						fromLeft,
						0,
						(window.innerHeight - bottomBarHeight) * (aspectRatio || window.screenAspectRatio),
				...
				canvas
					...
					.drawImage(
						recordedVideo,
						window.innerWidth - (1280 * 105) / 720 - 50,
						window.innerHeight - bottomBarHeight - 150,
						(1280 * 105) / 720,
						105
					);
			} else {
		...
		<div className="App">
			...
			<header className="App-header" id="content" style={{ overflow: 'hidden' }}>
				{!recording && !screenshare ? (
					<div style={{ marginTop: -bottomBarHeight, textAlign: 'center' }}>
						<img src={defaultImg} width="350px" height="auto" />
						<div className="circular text-center medium" style={{ marginTop: 35 }}>
				...
				<div
					className="overlay"
					style={{ background: recording || screenshare ? '#f5f5f538' : 'whitesmoke' }}
				></div>
				<div className="action-buttons">
					<div
						data-tip
						data-for="shareScreen"
						onClick={() => (screenshare ? stopScreenShare() : startScreenRecording())}
						className="circular start"
						id={screenshare ? 'stop' : 'start'}
					>
						<img
							src={screenshare ? screen : screenOut}
							width="30px"
							height="30px"
							className="v-middle"
							style={{ marginLeft: !screenshare && -2 }}
						/>
					</div>

					<ReactTooltip id="shareScreen" type="dark" place="top" effect="solid">
						<span className="circular" style={{ fontSize: 15 }}>
					...
					<div
						...
						className="circular start2"
						id={recording ? 'stop' : 'start'}
					>
					...
					<ReactTooltip id="recording" type="dark" place="top" effect="solid">
						<span className="circular" style={{ fontSize: 15 }}>
							{recording ? ' Stop video' : 'Share video'}
						</span>
					</ReactTooltip>
					...
					<div
						...
						data-for="audio"
						onClick={() => muteAudio()}
						className={`circular start3`}
						id={!muted ? 'stop' : 'start'}
					>
						<img src={!muted ? mic : mutedmic} width="30px" height="30px" className="v-middle" />
						<div style={{ marginTop: 19, fontWeight: 100, color: 'grey', fontSize: '0.72rem' }}>
							{muted && 'Muted'}
						</div>
					</div>
					...
					<ReactTooltip id="audio" type="dark" place="top" effect="solid">
						<span className="circular" style={{ fontSize: 15 }}>
							{!muted ? 'Mute micophone' : 'Unmute micophone'}
						</span>
					</ReactTooltip>
					...
					<div
						data-tip
						data-for="storing"
						onClick={() => (storing ? stopRecording(true) : startRecording())}
						className={`circular start3 ${!recording && !screenshare && !storing ? 'disabled' : null}`}
						id={storing ? 'stop' : 'start'}
					>
					...
					<ReactTooltip id="storing" type="dark" place="top" effect="solid">
						<span className="circular" style={{ fontSize: 15 }}>
							{storing ? 'Stop recording' : 'Start recording'}
						</span>
					</ReactTooltip>
				</div>
				{storing && (
					<div className="active-window circular">
						<div className="v-middle text-center regular active-window-text">
							<img src={expanded == 'recorded' ? video : screen} width="25px" height="25px" />
							<div style={{ marginTop: 5 }}>
								You are sharing your {expanded !== 'recorded' ? 'screen' : 'video'}
							</div>
						</div>
					</div>
				)}
				<video
					id="screen"
					className={`${expanded == 'recorded' && 'bottom-right'}`}
					autoPlay
					playsInline
					muted
				...
				<video
					id="recorded"
					className={`${expanded == 'screen' && 'bottom-right'}`}
					playsInline
					autoPlay
					muted
					style={{ position: 'absolute', visibility: 'hidden', height: 0 }}
				...
				{screenshare && recording && (
					<div
						style={{ right: 20, top: 20, zIndex: 22 }}
						className="switch v-middle"
						onClick={() => setExpanded(expanded == 'screen' ? 'recorded' : 'screen')}
					>
						<div className="switch-div">
							<img
								src="https://image.flaticon.com/icons/svg/125/125868.svg"
								width="20px"
								height="20px"
								className="v-middle"
								style={{ marginRight: 10, marginLeft: 3 }}
							/>
							<span className="circular v-middle" style={{ fontSize: 17, marginTop: -1 }}>
								Switch views
							</span>
						</div>
					</div>
				...
				{storing && (
					<div
						id="animate-flicker"
						style={{ left: 20, top: 8, zIndex: 22, position: 'fixed' }}
						title="Recording"
					>
				...
				<canvas
					style={{
						...
						position:
							!recording && !screenshare
								? 'absolute'
								: (!screenshare && recording) || (screenshare && window.expanded !== 'screen')
								? 'absolute'
								: 'null',
						/*left: window.expanded !== 'screen' && canvasPos.left,*/ top: 0,
					}}
				></canvas>
				{storing && (
					<div className="timer circular regular" style={{ color: '#f3f1f1' }}>
						<div style={{ display: 'inline-block' }}>{hours <= 9 ? '0' + hours : hours}: </div>
						<div style={{ display: 'inline-block' }}>
							{minutes <= 9 ? <span>0{minutes}</span> : minutes}:
						</div>
